<template>
  <div class="drag">
    <div
      :style="{ width: boxWidth === 'auto' ? 'auto' : boxWidth + 'px' }"
      :class="getmode"
    >
      <div
        :style="{
        ...
        class="box"
        v-show="isDrag"
        v-html="dragAry[dragInfo.index].html"
      ></div>
      <div
        :class="[
          setBlcock,
          isTargetDrag && dragIndex === index ? 'isTargetDrag' : '',
        ...
        :style="{
          background:
            dragMode && isDrag && index === Number(dragInfo.index)
              ? actInfo.color
              : item.color,
    ...
    dragMove(ev, index) {
      ...
      document.onmousemove = (moveEv) => {
        ...
        this.dragAry.forEach((item, indexs) => {
          const [{ offsetLeft: ox, offsetTop: oy }] =
            this.$refs["block" + indexs];
          if (
            moveX + offsetWidth / 2 > ox &&
            moveX + offsetWidth / 2 < ox + offsetWidth &&
            moveY + offsetHeight / 2 > oy &&
            moveY + offsetHeight / 2 < oy + offsetHeight
          ) {
            console.log(indexs);
            ...
            console.log(this.actInfo.html, "23123");
          }
        });
